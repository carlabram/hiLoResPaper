...
Figure \ref{fig:noStack} explores different kinds of $H=1$ mass growth histories, the prior behind
our Technique 2 SFH inferences (Section \ref{sec:h1}). These tracks are analogous to the suite of 
...
each independently normalized to $M(\tobs)=1$. The purple ensemble is the result one obtains
by fitting: in the case of one object or a stack, the luminosity and color of the data will confine 
...
Note that the mean of the inferred suite of histories is distinct from the mean of the native 
suite of histories, even though they are composed of exactly the same tracks. Stepping forward, 
as the galaxies do, results in a different outcome as stepping backwards---from an object or a stack
of a class of objects---as the fitter must \citep[see also][]{Behroozi13a, Torrey17}. In this context,
a physical question of interest would be ``Is $H=1$ stochasticity an approximation of the correct
SFH reference set for real galaxies?'' The answer to that question would be obtained by assessing 
whether {\it the difference between mean histories} is as shown here (using a non $H=1$ basis in
the inference). Since this difference is large, high precision inferences are not needed; many 
lower-quality measurements will do equally well.

The bottom panel shows a more intuitive example. We reproduce the purple tracks from the top panel split 
...
it is again clear that (1) the predicted difference in mean half mass times between passive and starforming
galaxies is large, such that precision isn't necessary, and (2) that objects of the same stellar mass {\it and} 
SFR---intuitive bin parameters---may have half-mass times differing by 40\% of a Hubble time. What 
the average of these tracks---at any precision---says about the physics causing any one of them to behave 
as it did is hard to see.

The issue is that there are only two objects of physical interest: good assessments of (1) individual SFHs, 
or (2) the typical SFH of a meaningfully defined class of galaxies. Due to $S/N$ requirements, (1) is not 
obtainable. Meanwhile, (2) is an average over {\it histories}, and so is assessable by combining many 
low-fidelity inferences for individual objects.\footnote{It is
...
only proxy for this, but it is not in reality the object of interest.} Both statements suggest the pursuit of data at sufficient $S/N$ 
to yield $1\%$ line strength estimates---either directly or via stacking---is unnecessary. %This is what Figure \ref{fig:noStack}, bottom, shows.
